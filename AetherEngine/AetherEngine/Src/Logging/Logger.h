...
#pragma endregion
		...
		/// <returns></returns>
		Logger(std::string threadName = "Main", uint8_t severity = 0);
		~Logger();
	...
	private:
		static uint8_t severityLevel; // 0 = nothing, 1 = error, 2 = error & warning, 3 = error & warning & info, 4 = error & warning & info & verbose
		static bool addTags;
